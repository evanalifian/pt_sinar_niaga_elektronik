<?php include("../../layouts/header.php"); ?>

<h3>Daftar Pegawai</h3>

<a href="add.php" class="btn btn-primary mb-3">Tambah Pegawai</a>

    <table class="table table-bordered">
        <thead>
            <tr>
                <th>ID Pegawai</th>
                <th>Nama Pegawai</th>
                <th>Jabatan</th>
                <th>Tanggal Masuk</th>
                <th>Jenis Kelamin</th>
                <th>Tanggal Lahir</th>
                <th>Alamat</th>
                <th>No Telepon</th>
                <th>Email</th>
                <th>Gaji</th>
                <th>Cabang</th>
                <th>Aksi</th>
            </tr>
        </thead>
        <tbody>
            <?php
            // Ambil pegawai + nama cabangnya
            $q = mysqli_query($conn, "
            SELECT pegawai.*, cabang.nama_cabang 
            FROM pegawai 
            JOIN cabang ON pegawai.kode_cabang = cabang.kode_cabang
        ");

            while ($row = mysqli_fetch_assoc($q)) {
                echo "
            <tr>
                <td>{$row['id_pegawai']}</td>
                <td>{$row['nama_pegawai']}</td>
                <td>{$row['jabatan']}</td>
                <td>{$row['tgl_masuk_pegawai']}</td>
                <td>{$row['jenis_kelamin_pegawai']}</td>
                <td>{$row['tanggal_lahir_pegawai']}</td>
                <td>{$row['alamat_pegawai']}</td>
                <td>{$row['no_telepon_pegawai']}</td>
                <td>{$row['email_pegawai']}</td>
                <td>{$row['gaji_pegawai']}</td>
                <td>{$row['nama_cabang']}</td>
                <td>
                    <a href='edit.php?id={$row['id_pegawai']}' class='btn btn-warning btn-sm'>Edit</a>
                    <a href='delete.php?id={$row['id_pegawai']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Hapus pegawai?\")'>Hapus</a>
                </td>
            </tr>";
            }
            ?>
        </tbody>
    </table>

<?php include("../../layouts/footer.php"); ?>